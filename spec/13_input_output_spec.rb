...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_ongoing) { described_class.new(5, '4') }
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        first_attempt = 'inv4lid'
        second_attempt = 'hu3'
        valid_input = '1'
        allow(game_loop)
          .to receive(:player_input)
          .and_return(first_attempt, second_attempt, valid_input)
      end

      it 'completes loop and displays error message twice' do
        expect(game_loop).to receive(:puts).and_return('Input error!').twice
        game_loop.player_turn
      end
    end
  end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game) { described_class.new(1, '2', 3) }
    ...
    context 'when count is 4 and over' do
      subject(:game) { described_class.new(1, '2', count) }

      let(:count) { 4 }

      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        count_phrase = "That was hard. It took you #{count} guesses!\n"
        expect{ game.final_message }.to output(count_phrase).to_stdout
      end
    end
  end
end
